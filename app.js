...
function formatGregorianDate(dateInput) {
    ...
    try {
        ...
        // التأكد من أن التاريخ صالح قبل المتابعة
...
// ==========================================================
/**
 * دالة لحساب تفاصيل الوردية القادمة لموظف
 * @param {object} schedule - كائن يحتوي على تفاصيل الوردية (days, start_time, end_time)
...
function calculateNextShift(schedule) {
    ...
    if (!nextShiftStartDate) {
        for (let i = 7; i < 14; i++) {
            ...
            const checkDay = checkDate.getDay();
            if (workDays.includes(checkDay)) {
                nextShiftStartDate = new Date(checkDate);
                nextShiftStartDate.setHours(startHours, startMinutes, 0, 0);
                break;
            }
        }
    }
...
// ==========================================================
// ==========================================================
// ===       بداية دالة تصدير الشواغر إلى Excel احترافي      ===
// ==========================================================
...
// ==========================================================
// ===      بداية الإضافة: تعريف متغيرات الخريطة العامة      ===
// ==========================================================
...
async function loadOpsReviewRequestsPage() {
    ...
    const { data: requests, error } = await supabaseClient
        ...
        .eq('status', 'معلق')
        .not('request_type', 'eq', 'permission') // <-- هذا هو السطر الجديد الذي يمنع ظهور الاستئذان
    ...
    // ==========================================================
    // ===        بداية الإضافة: تفعيل الاشتراك المباشر         ===
    // ==========================================================
    activeSubscription = supabaseClient
        ...
        .on(
            ...
            (payload) => {
                console.log('Realtime update from employee_requests table for OPS!', payload);
...
// هذا الكود سيفترض وجود دالة initializeMap وعناصر الخريطة لديك جاهزة.
 // يرجى التأكد من أن لديك Leaflet JavaScript ومكتبة الخرائط الأساسية مضمنة في مشروعك.

 async function viewPatrolPath(patrolId) {
    // الانتقال إلى صفحة الخريطة أولاً
    document.querySelector('a[data-page="page-geo"]').click();

    // إعطاء مهلة بسيطة للخريطة للتحميل قبل الرسم
    setTimeout(async () => {
        try {
            ...
            // مسح أي علامات أو مسارات قديمة على الخريطة
            ...
            // تعريف أيقونات البداية والنهاية والتوقف
            ...
            // --- بداية منطق تحليل وتحديد نقاط التوقف ---
            ...
            // --- نهاية منطق تحليل التوقف ---

            // إضافة علامات البداية والنهاية (تُضاف أخيراً لتظهر في الأعلى)
            if (startPoint) {
 ...
 // تحتاج إلى تعديل الدالة التي تستدعي هذه الوظيفة (viewPatrolPath)
 // في حدث النقر على زر "عرض المسار" لتمرير معرف الجولة الصحيح.
...
async function loadGuardAttendancePage() {
    ...
    try {
        ...
        if (currentUser.role === 'مشرف') {
            coverageQuery = createProjectFilter(coverageQuery, currentUser.project, 'project');
            vacancyQuery = createProjectFilter(vacancyQuery, currentUser.project);
        } else if (currentUser.role === 'ادارة العمليات') {
            coverageQuery = coverageQuery.eq('region', currentUser.region);
        ...
        if (!activeSubscription) {
            activeSubscription = supabaseClient
                ...
                .on('postgres_changes', { event: '*', schema: 'public', table: 'attendance' }, () => loadGuardAttendancePage())
...
async function loadAdminDashboardPage() {
    ...
    adminPage.addEventListener('click', (event) => {
        ...
        // ==================== بداية الإضافة: إيقاف المؤقت التلقائي عند التنقل ====================
...
function updateUIVisibility(role) {
    ...
    allMenuItems.forEach(item => {
        ...
        // ================================================================
        // هذا الشرط يتحقق من الصفحة الجديدة بشكل خاص
        if (page === 'page-attendance-management') {
            // تحقق إذا كان المستخدم مدير نظام، أو موارد بشرية ولديه الصلاحية
            const canView = (
                currentUser.role === 'مدير النظام' || 
                (currentUser.role === 'ادارة الموارد البشرية' && currentUser.can_edit_attendance)
            );

            item.style.display = canView ? 'block' : 'none';

        // ================================================================
        // ===    نهاية الإضافة / الكود الأصلي لباقي الصفحات         ===
        // ================================================================
        } else {
            // الكود الأصلي لباقي الصفحات لم يتغير
...
// دالة جديدة (متزامنة) لإظهار/إخفاء الحقول فوراً
...
// دالة جديدة (غير متزامنة) لجلب البيانات وتعبئة الحقول
async function populateManagerialFields(employeeData) {
    const role = employeeData.role;

    if (role === 'ادارة العمليات' || role === 'مشرف') {
        document.getElementById('assign-region-select').value = employeeData.region || '';
    }
    ...
    if (role === 'مشرف') {
        ...
        if (contracts) {
            ...
            const supervisorProjects = Array.isArray(employeeData.project) ? employeeData.project : [];
    ...
    try {
        ...
        if (latestRecord && !latestRecord.checkout_at) {
            ...
            if (schedule) {
                ...
                if (schedule.days && schedule.days.includes(todayKey)) {
                    const [startHours, startMinutes] = schedule.start_time.split(':').map(Number);
                    const [endHours, endMinutes] = schedule.end_time.split(':').map(Number);
                    ...
                    const gracePeriodStart = new Date(shiftStartDate.getTime() - 15 * 60000);

                    if (now >= gracePeriodStart && now < shiftEndDate) {
...
async function loadMapStatistics(filters = {}) {
    ...
    try {
        ...
        if (filters.region) guardsQuery.eq('region', filters.region);
        if (filters.city) guardsQuery.eq('city', filters.city);
        if (filters.project) createProjectFilter(guardsQuery, [filters.project]);
        if (filters.location) guardsQuery.eq('location', filters.location);
...
async function loadUserManagementPage() {
    ...
    // جلب العمود الجديد 'can_edit_attendance' مع باقي البيانات
    const { data: users, error } = await supabaseClient.from('users').select('*, can_edit_attendance').order('name', { ascending: true });
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>المشروع</th>
                    <th>الحالة</th>
                    <th>تعديل الحضور</th> <th>إجراءات</th>
                </tr>
            </thead>
            <tbody>
                ${users.map(user => {
                    // التحقق إذا كان الموظف من الموارد البشرية لعرض مربع الاختيار
                    const permissionCheckbox = user.role === 'ادارة الموارد البشرية'
                        ? `<input type="checkbox" class="attendance-permission-checkbox" data-user-id="${user.id}" ${user.can_edit_attendance ? 'checked' : ''}>`
                        : '-';
                    ...
                    return `
                        <tr>
                            ...
                            <td style="text-align: center;">${permissionCheckbox}</td> <td>
...
async function initializeMap() {
    ...
    try {
        let query = supabaseClient
            .from('attendance')
            .select(`
                users:guard_id!inner(
                    id, name, project, location, city, region,
                    guard_locations!inner(latitude, longitude, created_at),
        ...
        if (filters.project) query = createProjectFilter(query, [filters.project], 'users.project');
...
// ==========================================================
// ===       بداية الإضافة: تفعيل مؤقت تحديث الإحصائيات       ===
// ==========================================================
// إيقاف أي مؤقت قديم لضمان عدم وجود مؤقتات متعددة تعمل
if (statsInterval) clearInterval(statsInterval);
// تشغيل مؤقت جديد يقوم بتحديث الإحصائيات كل 60 ثانية
...
async function loadPenaltiesPage() {
    ...
    // --- قراءة قيم الفلاتر الجديدة من الواجهة ---
    const searchVal = document.getElementById('hr-filter-search-penalties').value;
    const regionVal = document.getElementById('hr-filter-region-penalties').value;
    const cityVal = document.getElementById('hr-filter-city-penalties').value;
    const projectVal = document.getElementById('hr-filter-project-penalties').value;
    const locationVal = document.getElementById('hr-filter-location-penalties').value;

    let query = supabaseClient.from('users').select('id, name, role, project');

    // --- تطبيق الفلاتر على استعلام قاعدة البيانات ---
    if (searchVal) {
    ...
    if (employees.length === 0) {
        container.innerHTML = '<p style="text-align: center;">لم يتم العثور على موظفين يطابقون شروط الفلترة.</p>';
        return;
    }
    ...
    try {
        ...
        if (contract && contract.contract_locations) {
            ...
            if (locationData && locationData.geofence_link) {
                const coords = locationData.geofence_link.replace(/\s/g, '');
                const userAgent = navigator.userAgent;
                if (/iPad|iPhone|iPod/.test(userAgent)) {
                    locationLink = `comgooglemaps://?q=${coords}&center=${coords}&zoom=15`;
        ...
        weekDaysOrder.forEach((dayKey, index) => {
            ...
            if (dayData && locationLink) {
                 scheduleHtml += `<div class="schedule-card-footer"><a href="${locationLink}" target="_blank" class="btn btn-secondary btn-sm"><i class="ph-bold ph-map-pin"></i> عرض الموقع</a></div>`;
            }
            scheduleHtml += '</div>';
        });
        scheduleHtml += '</div>';
...
async function loadSupervisorPermissionRequestsPage() {
    ...
    const { data: requests, error } = await supabaseClient
        ...
        .eq('request_type', 'permission')
        .eq('status', 'معلق')
        .filter('users.project', 'ov', `{${currentUser.project.join(',')}}`);
    // --- نهاية التصحيح ---
...
async function loadPatrolsHistory() {
    ...
    try {
        ...
        // إضافة فلتر الصلاحيات بناءً على دور المستخدم
        if (currentUser.role === 'ادارة العمليات') {
            query = query.eq('supervisor.region', currentUser.region);
        } else if (currentUser.role === 'مشرف') {
            // استخدام الدالة المساعدة لفلترة مشاريع المشرف
...
async function loadVacancyTabData() {
    ...
    try {
        // --- 1. قراءة قيم الفلاتر ---
        const regionVal = document.getElementById('vc-filter-region').value;
        const cityVal = document.getElementById('vc-filter-city').value;
        const projectVal = document.getElementById('vc-filter-project').value;
        const locationVal = document.getElementById('vc-filter-location').value;
        const statusVal = document.getElementById('vc-filter-status').value;

        // --- 2. جلب البيانات الأساسية ---
        let query = supabaseClient.from('job_vacancies').select('*, contracts(company_name)');

        // تطبيق فلاتر الموقع على استعلام قاعدة البيانات
        if (regionVal) query = query.eq('region', regionVal);
        if (cityVal) query = query.eq('location', cityVal); // 'location' in vacancies is city
        if (projectVal) query = query.eq('project', projectVal);
        if (locationVal) query = query.eq('specific_location', locationVal);

        const { data: vacancies, error: vacanciesError } = await query.order('created_at', { ascending: false });
        ...
        const finalVacancies = vacancies.filter(vacancy => {
            if (!statusVal) return true; // إذا لم يتم اختيار فلتر الحالة، أظهر الكل
            const isAssigned = assignedUsers.some(u => u.vacancy_id === vacancy.id);
        ...
        const totalRequired = finalVacancies.reduce((sum, vacancy) => {
            const shift = vacancy.schedule_details?.[0];
            return sum + (parseInt(shift?.guards_count) || 1);
        }, 0);
        const totalAssigned = finalVacancies.filter(v => assignedUsers.some(u => u.vacancy_id === v.id)).length;
        ...
        vacanciesExportData = finalVacancies.map(vacancy => {
            ...
            if (vacancy.status === 'open') {
                ...
                statusText = assignedUser ? 'مغلق (مرتبط بشخص)' : 'مغلق (فارغ)';
            }
            ...
            return {
                ...
                shift: shift ? `${shift.name || 'وردية'} (${formatTimeAMPM(shift.start_time)} - ${formatTimeAMPM(shift.end_time)})` : 'غير محدد',
...
// دالة لجلب وتنظيم بيانات الفلاتر من العقود
async function getFilterData() {
    if (hrFilterData) return hrFilterData; 

    const { data: contracts, error } = await supabaseClient.from('contracts').select('region, city, company_name, contract_locations');
    if (error) {
    ...
    contracts.forEach(contract => {
        ...
        cities.forEach(city => {
            ...
            (contract.contract_locations || []).forEach(loc => {
                if (loc.city === city) {
                    structuredData[region][city][project].add(loc.name);
                }
            });
        });
    });
...
// دالة عامة لملء القوائم المنسدلة
async function initializeFilters(pagePrefix) {
    const data = await getFilterData();
    const regionSelect = document.getElementById(`${pagePrefix}-filter-region`);
...
// بداية الإضافة
// دالة خاصة لتهيئة فلاتر صفحة حضور الحراس (للمشرفين)
async function initializeGuardAttendanceFilters() {
    if (currentUser.role === 'مشرف') {
        ...
        // استدعاء الدالة الجديدة التي أنشأناها في قاعدة البيانات
        ...
        // إظهار فلتر المنطقة لباقي الأدوار
...
// دالة لتحديد أي صفحة يجب إعادة تحميلها بناءً على الفلتر
// بداية الاستبدال
...
// منطق تشغيل الفلاتر المترابطة (Cascading)
document.addEventListener('change', async function(event) {
    const target = event.target;
    const isSupervisorOnAttendancePage = target.classList.contains('cascading-filter-ga') && currentUser.role === 'مشرف';

    // --- 1. منطق خاص للمشرف في صفحة حضور الحراس ---
    if (isSupervisorOnAttendancePage) {
        const cityVal = document.getElementById('ga-filter-city').value;
        const projectSelect = document.getElementById('ga-filter-project');
        ...
        if (target.id === 'ga-filter-city' && cityVal) {
            ...
            for (const region in data) {
                if (data[region][cityVal]) {
                    Object.keys(data[region][cityVal]).forEach(project => {
                        if (supervisorProjects.includes(project)) {
                            projectsInCity.add(project);
                        }
                    });
                }
            }
            ...
            if (uniqueProjects.length > 0) {
                uniqueProjects.forEach(p => projectSelect.innerHTML += `<option value="${p}">${p}</option>`);
            ...
            if (locations.length > 0) {
                locations.sort().forEach(l => locationSelect.innerHTML += `<option value="${l}">${l}</option>`);
    ...
    // --- 2. المنطق العام لبقية المستخدمين والصفحات ---
    const pagePrefixes = { 'ga-': 'ga', 'geo-': 'geo', 'vc-': 'vc' };
    let pagePrefix = null;
    for (const p in pagePrefixes) {
        if (target.id.startsWith(p)) {
            pagePrefix = pagePrefixes[p];
            break;
        }
    }
    if (!pagePrefix && target.id.startsWith('hr-')) pagePrefix = 'hr-' + target.id.split('-').pop();

    if (target.classList.contains('cascading-filter') || target.classList.contains('cascading-filter-ga') || target.classList.contains('cascading-filter-geo') || target.classList.contains('cascading-filter-vc')) {
        const nextSelectId = target.dataset.target;
        if (!nextSelectId) { triggerDataLoad(pagePrefix); return; }
        const nextSelect = document.getElementById(nextSelectId);

        let current = nextSelect;
        while(current) {
            current.disabled = true;
            current.innerHTML = `<option value="">الكل</option>`;
            const nextTargetId = current.dataset.target;
            current = nextTargetId ? document.getElementById(nextTargetId) : null;
        }

        const data = await getFilterData();
        const regionVal = document.getElementById(`${pagePrefix}-filter-region`).value;
        const cityVal = document.getElementById(`${pagePrefix}-filter-city`) ? document.getElementById(`${pagePrefix}-filter-city`).value : null;
        const projectVal = document.getElementById(`${pagePrefix}-filter-project`) ? document.getElementById(`${pagePrefix}-filter-project`).value : null;

        let options = [];
        if (nextSelectId.includes('city')) { 
            if (regionVal && data[regionVal]) options = Object.keys(data[regionVal]).sort();
        } else if (nextSelectId.includes('project')) { 
            if (regionVal && cityVal && data[regionVal][cityVal]) options = Object.keys(data[regionVal][cityVal]).sort();
        } else if (nextSelectId.includes('location')) {
            if (regionVal && cityVal && projectVal && data[regionVal][cityVal][projectVal]) options = data[regionVal][cityVal][projectVal].sort();
        }

        if (options.length > 0) {
            nextSelect.disabled = false;
            options.forEach(opt => nextSelect.innerHTML += `<option value="${opt}">${opt}</option>`);
        }
...
// منطق لإعادة تحميل البيانات عند البحث أو تغيير أي فلتر
document.addEventListener('input', function(event) {
    const target = event.target;
    if (target.id.includes('-search')) {
        const pagePrefix = target.id.split('-')[0];
        triggerDataLoad(pagePrefix);
    }
});
...
document.addEventListener('change', function(event) {
    ...
    if (target.classList.contains('ga-filter-control') && !target.classList.contains('cascading-filter-ga')) triggerDataLoad('ga');
     else if (target.classList.contains('vc-filter-control')) triggerDataLoad('vc'); // <-- الإضافة الجديدة
    if (target.classList.contains('geo-filter-control') && !target.classList.contains('cascading-filter-geo')) triggerDataLoad('geo');
    if (target.classList.contains('hr-filter-control')) triggerDataLoad('hr-' + target.id.split('-').pop());
});
...
// ==========================================================
// ==========================================================
// ===       بداية الإضافة: منطق الفلاتر المترابطة        ===
// ==========================================================
...
// معالج حدث شامل لمراقبة أي تغيير في فلاتر الموارد البشرية
document.body.addEventListener('change', async function(event) {
    ...
    // إعادة تحميل البيانات عند تغيير أي فلتر
    if (event.target.classList.contains('hr-filter-control')) {
        if (document.querySelector('#page-employees:not(.hidden)')) {
            loadEmployeeTabData();
        }
        // يمكن إضافة الصفحات الأخرى هنا مستقبلاً
    }
});

// إعادة تحميل البيانات عند الكتابة في صندوق البحث
...
async function loadEmployeeTabData() {
    ...
    // --- قراءة قيم جميع الفلاتر من الواجهة الجديدة ---
    ...
    let query = supabaseClient.from('users').select(`id, name, role, project, city, region, location, phone, employment_status, auth_user_id`);
    ...
    if (projectVal) {
        // .filter مع 'cs' تستخدم للبحث داخل حقل من نوع مصفوفة (array)
        query = query.filter('project', 'cs', `{${projectVal}}`);
    }
    if (locationVal) {
        query = query.eq('location', locationVal);
    ...
    // --- باقي الدالة (لإنشاء الجدول) يبقى كما هو ---
    ...
    console.log('Subscribed to real-time users table.');
    // ==========================================================
    // ===                 نهاية الإضافة                     ===
    // ==========================================================
}
...
// بداية الاستبدال
// ========= بداية الاستبدال الكامل للدالة =========
// ========= بداية الاستبدال الكامل للدالة (مع الفلاتر الذكية) =========
async function loadHrAttendanceLogPage() {
    const container = document.getElementById('hr-attendance-log-container');
    container.innerHTML = '<p style="text-align: center;">جاري تحميل آخر السجلات...</p>';

    try {
        const dateFrom = document.getElementById('att-log-date-from').value;
        const dateTo = document.getElementById('att-log-date-to').value;
        ...
        const hasActiveFilters = dateFrom || dateTo || status || region || city || project || location;

        let query = supabaseClient.from('attendance').select(`*, users!inner(name, region, project, location, city)`).order('created_at', { ascending: false });

        // إذا لم تكن هناك فلاتر نشطة، اعرض آخر 50 سجل فقط
        if (!hasActiveFilters) {
        ...
        if (region) query = query.eq('users.region', region);
        if (city) query = query.eq('users.city', city);
        if (project) query = query.filter('users.project', 'cs', `{${project}}`);
        if (location) query = query.eq('users.location', location);

        const { data, error } = await query;
        if (error) throw error;

        if (data.length === 0) {
            container.innerHTML = `<p style="text-align: center;">${hasActiveFilters ? 'لا توجد سجلات تطابق البحث.' : 'لا توجد سجلات حضور لعرضها.'}</p>`;
            return;
        }
        
        const groupedData = data.reduce((acc, record) => {
            if (!record.users) return acc;
            const projectDisplay = Array.isArray(record.users.project) ? record.users.project.join(', ') : record.users.project;
            const key = `${projectDisplay || 'غير محدد'} > ${record.users.location || 'غير محدد'}`;
            if (!acc[key]) acc[key] = [];
            acc[key].push(record);
            return acc;
        }, {});
        ...
        const statusClasses = {
            'حاضر': 'active',
            'اكمل المناوبة': 'completed',
            'غياب': 'inactive',
            'انسحاب': 'inactive',
            'استئذان': 'permission',
            'تم تسجيل الخروج تلقائياً': 'completed'
        };

        let tableHtml = `<table class="attendance-log-table"><thead><tr><th>الحارس</th><th>الحضور</th><th>الانصراف</th><th>مدة الوردية</th><th>الحالة</th></tr></thead><tbody>`;

        for (const groupName in groupedData) {
            tableHtml += `<tr class="location-header-row"><th colspan="5">${groupName}</th></tr>`;
            const records = groupedData[groupName];
            records.forEach(r => {
                let durationHtml = '-';
                if (r.created_at && r.checkout_at) {
                    const checkinTime = new Date(r.created_at);
                    const checkoutTime = new Date(r.checkout_at);
                    const diffMs = checkoutTime - checkinTime;
                    const hours = Math.floor(diffMs / 3600000);
                    const minutes = Math.floor((diffMs % 3600000) / 60000);
                    durationHtml = `${hours} س و ${minutes} د`;
                }
                const statusClass = statusClasses[r.status] || 'pending';
                tableHtml += `
                    <tr>
                        <td>${r.users.name}</td>
                        <td>${formatGregorianDateTime(r.created_at)}</td>
                        <td>${r.checkout_at ? formatGregorianDateTime(r.checkout_at) : '-'}</td>
                        <td>${durationHtml}</td>
        ...
        // ==========================================================
        activeSubscription = supabaseClient
            .channel('public:attendance:log')
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'attendance' },
                (payload) => {
                    console.log('Realtime update for HR attendance log!', payload);
                    loadHrAttendanceLogPage();
                }
            )
            .subscribe();

        console.log('Subscribed to real-time HR attendance log.');
        // ==========================================================
        // ===                 نهاية الإضافة                     ===
        // ==========================================================
    } catch (err) {
...
// ========= بداية الاستبدال الكامل لدالة generatePayroll (مع الفلاتر الذكية) =========
async function generatePayroll() {
    ...
    try {
        ...
        const [ 
            ...
            supabaseClient.from('penalties').select('user_id, amount, reason').gte('deduction_date', startDateString).lte('deduction_date', endDateString),
        ...
        if (allEmployees.length === 0) {
            resultsContainer.innerHTML = '<p style="text-align: center;">لم يتم العثور على موظفين يطابقون شروط الفلترة.</p>';
            return;
        }
...
// ==========================================================
// ===      بداية الإضافة: دالة صفحة إدارة الحضور        ===
// ==========================================================
async function loadAttendanceManagementPage() {
    ...
    try {
        // --- قراءة قيم الفلاتر الجديدة من الواجهة ---
        const searchVal = document.getElementById('hr-filter-search-attmgmt').value;
        const dateFrom = document.getElementById('hr-filter-date-from-attmgmt').value;
        const dateTo = document.getElementById('hr-filter-date-to-attmgmt').value;
        const region = document.getElementById('hr-filter-region-attmgmt').value;
        const city = document.getElementById('hr-filter-city-attmgmt').value;
        const project = document.getElementById('hr-filter-project-attmgmt').value;
        const location = document.getElementById('hr-filter-location-attmgmt').value;

        let query = supabaseClient
            .from('attendance')
            .select(`*, users!inner(name, region, project, city, location)`) // !inner لربط الفلاتر ببيانات الموظف
            .order('created_at', { ascending: false });

        // --- تطبيق الفلاتر على الاستعلام ---
        if (searchVal) {
            query = query.or(`users.name.ilike.%${searchVal}%,users.id_number.ilike.%${searchVal}%`);
        }
        if (dateFrom) {
        ...
        // إذا لم تكن هناك أي فلاتر نشطة، اعرض آخر 50 سجل فقط
        const activeFilters = [searchVal, dateFrom, dateTo, region, city, project, location].filter(Boolean);
        ...
        container.innerHTML = `
            <table>
                ...
                <tbody>
                    ${data.map(record => `
                        <tr>
                            <td>${record.users ? record.users.name : 'غير معروف'}</td>
                            <td>${formatGregorianDateTime(record.created_at)}</td>
                            <td>${record.checkout_at ? formatGregorianDateTime(record.checkout_at) : '-'}</td>
                            <td><span class="status ${record.status === 'حاضر' ? 'active' : 'inactive'}">${record.status}</span></td>
                            <td>
                                <button class="btn btn-secondary btn-sm att-mgmt-edit-btn" 
                                        data-id="${record.id}" 
                                        data-name="${record.users ? record.users.name : 'غير معروف'}"
                                        title="تعديل السجل">
    ...
    // ==========================================================
        ...
        // ==========================================================
        activeSubscription = supabaseClient
            .channel('public:attendance:management')
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'attendance' },
                (payload) => {
                    console.log('Realtime update for attendance management!', payload);
                    loadAttendanceManagementPage();
                }
            )
            .subscribe();

        console.log('Subscribed to real-time attendance management.');
        // ==========================================================
        // ===                 نهاية الإضافة                     ===
        // ==========================================================
    } catch (err) {
        container.innerHTML = `<p style="color: red; text-align:center;">حدث خطأ: ${err.message}</p>`;
        console.error("Attendance Management Page Error:", err);
    }
}
...
navLinks.forEach(link => {
    ...
    loadEmployeeTabData();
    initializeFilters('employees'); // تهيئة الفلاتر عند فتح الصفحة لأول مرة
}
    ...
    loadAttendanceManagementPage();
    initializeFilters('attmgmt'); // تهيئة الفلاتر عند فتح الصفحة
}
    ...
    loadHrAttendanceLogPage();
    initializeFilters('attlog'); // تهيئة الفلاتر عند فتح الصفحة
}
    ...
    initializeFilters('payroll'); // تهيئة الفلاتر عند فتح الصفحة لأول مرة
}
...
if (document.querySelector('#page-attendance-management:not(.hidden)')) loadAttendanceManagementPage(); // <-- السطر الجديد
    }
...
// ================================================================
// ===      بداية الإضافة: منطق صلاحية تعديل الحضور        ===
// ================================================================
const permissionCheckbox = event.target.closest('.attendance-permission-checkbox');
if (permissionCheckbox) {
    ...
    // تعطيل المربع مؤقتاً لمنع النقرات المزدوجة
    permissionCheckbox.disabled = true;

    try {
        const { error } = await supabaseClient
            .from('users')
            .update({ can_edit_attendance: newState })
            .eq('id', userId);

        if (error) throw error;

        // تسجيل الإجراء في سجل الأحداث
        await supabaseClient.from('audit_logs').insert({
            ...
            details: {
                ...
                action: `${actionText} صلاحية تعديل الحضور`
            }
        });
        ...
        // في حالة الخطأ، أعد المربع إلى حالته السابقة
        permissionCheckbox.checked = !newState;
    } finally {
        // إعادة تفعيل المربع بعد انتهاء العملية
...
// ==========================================================
// ==========================================================
// ===      بداية الاستبدال: منطق تسجيل الحضور الذكي       ===
// ==========================================================
if (event.target.closest('#check-in-btn')) {
    ...
    // دالة مساعدة للحصول على الموقع مع مهلة زمنية
    ...
    // دالة لإعادة الواجهة للحالة الطبيعية
    const resetUI = () => {
    ...
    try {
        ...
        const { data: vacancy, error: e1 } = await supabaseClient.from('job_vacancies').select('contract_id, project, location, region, specific_location, schedule_details').eq('id', currentUser.vacancy_id).single();
        if (e1 || !vacancy) throw new Error('لم يتم العثور على بيانات الشاغر الوظيفي.');
        ...
        if (e2 || !contract) throw new Error('لا يمكن العثور على بيانات العقد.');
        const locationData = contract.contract_locations.find(loc => loc.name === vacancy.specific_location);
        if (!locationData || !locationData.geofence_link) throw new Error('لم يتم تحديد إحداثيات الموقع في العقد.');
        const siteCoords = parseCoordinates(locationData.geofence_link);
        const radius = locationData.geofence_radius || 200;
        ...
        // --- 2. بدء عملية البحث الذكي ---
        checkInBtn.disabled = true;
        attendanceStatus.innerHTML = `
            <p style="color: var(--accent-color);">جاري البحث عن إشارة GPS دقيقة...</p>
            <p style="font-size: 0.9rem; color: var(--text-secondary);">الرجاء البقاء في مكانك لثوانٍ.</p>
        ...
        const MAX_ATTEMPTS = 6; // 6 محاولات * 10 ثواني = 60 ثانية كحد أقصى
        const PRIMARY_ACCURACY = 150;
        const SECONDARY_ACCURACY = 200; // الدقة المقبولة بعد 4 محاولات

        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            if (isCheckinCancelled) break;

            try {
                const position = await getLocation(10000); // مهلة 10 ثواني لكل محاولة
                ...
                const isPrimarySuccess = accuracy < PRIMARY_ACCURACY && distance <= radius;
                const isSecondarySuccess = attempt > 4 && accuracy < SECONDARY_ACCURACY && distance <= radius;

                if (isPrimarySuccess || isSecondarySuccess) {
                    const { error: insertError } = await supabaseClient.from('attendance').insert({
                        guard_id: currentUser.id, guard_name: currentUser.name, vacancy_id: currentUser.vacancy_id, project: vacancy.project,
                        location: vacancy.specific_location, region: vacancy.region, city: vacancy.location,
                        checkin_lat: guardCoords.lat, checkin_lon: guardCoords.lng, status: 'حاضر'
                    });
                    if (insertError) throw insertError;

                    showCustomAlert('نجاح', `تم تسجيل حضورك بنجاح بدقة ${accuracy.toFixed(0)} متر.`, 'success');
                    loadAttendancePage();
                    checkinSuccessful = true;
                    break;
                } else if (accuracy < SECONDARY_ACCURACY && distance > radius) {
                    throw new Error(`أنت خارج نطاق العمل. المسافة الحالية: ${Math.round(distance)} متر.`);
                }

            } catch (geoError) {
                if (geoError.code === 1) throw new Error("يرجى السماح بالوصول للموقع لتسجيل الحضور.");
            }
        }

        if (!checkinSuccessful && !isCheckinCancelled) {
            throw new Error("تعذر الحصول على إشارة GPS دقيقة. يرجى المحاولة من مكان أكثر انفتاحاً.");
        }
...
if (event.target.closest('.edit-employee-btn')) {
    ...
    try {
        ...
        // الخطوة أ: إظهار الحقول الصحيحة فوراً وبدون تأخير
        toggleManagerialFields(employee.role);
        ...
        // الخطوة ب: جلب البيانات وتعبئة الحقول التي تحتاج وقت
        await populateManagerialFields(employee);
...
// زر البحث في صفحة سجل الحضور (هذا هو المكان الصحيح له)
    ...
    // --- بداية الإضافة: تفعيل فلاتر صفحة إدارة الحضور ---
        ...
        // ================================================================
// ===       بداية الإضافة: تفعيل زر التعديل في صفحة إدارة الحضور       ===
// ================================================================
...
if (attMgmtEditBtn) {
    ...
    // دالة مساعدة لتحويل التوقيت لصيغة يفهمها حقل الإدخال
    const toLocalISOString = (date) => {
        if (!date) return '';
        const d = new Date(date);
        d.setMinutes(d.getMinutes() - d.getTimezoneOffset());
    ...
    // جلب أحدث بيانات السجل من قاعدة البيانات قبل عرضها
...
// بداية الإضافة
    // --- منطق زر مسح فلتر الوقت في صفحة حضور الحراس ---
        ...
        if (action === 'reject') {
            ...
            switch (stage) {
                ...
                case 'hr_final':
                    ...
                    if (requestType === 'loan') {
                        // جلب تفاصيل الطلب لمعرفة مبلغ السلفة
                        const { data: requestDetails, error: detailsError } = await supabaseClient
                            .from('employee_requests')
                            .select('details')
                            .eq('id', requestId)
                            .single();
                        ...
                        if (!detailsError && requestDetails) {
                            // إنشاء سجل خصم تلقائي في جدول العقوبات/الخصومات
                            await supabaseClient.from('penalties').insert({
                                user_id: userId,
                                amount: parseFloat(requestDetails.details.amount),
                                reason: 'خصم سلفة معتمدة',
                                deduction_date: new Date().toISOString().split('T')[0] // تاريخ اليوم
                            });
                            successMessage += ' وتم تسجيلها كخصم في مسير الرواتب.';
                        }
                    }
                ...
                case 'direct_permission':
                    ...
                    // 3. إذا وجد سجل مفتوح، قم بتحديثه
                    if (openAttendance) {
                        await supabaseClient
                            .from('attendance')
                            .update({
...
if (event.target.closest('.edit-employee-btn')) {
    ...
    try {
        ...
        document.getElementById('employee-phone').value = employee.phone || '';
        document.getElementById('employee-role').value = employee.role || 'حارس أمن';
        document.getElementById('employee-role').dispatchEvent(new Event('change'));
        document.getElementById('employee-start-date').value = employee.start_of_work_date;
        document.getElementById('employee-password').value = '';
        document.getElementById('employee-password').placeholder = 'اتركه فارغاً لعدم التغيير';
        document.getElementById('employee-iban').value = employee.iban || '';
...
// بداية الاستبدال
await toggleManagerialFields(employee.role, employee);
// نهاية الاستبدال
...
// --- بداية منطق إرسال الرسائل من صفحة الموارد البشرية ---
